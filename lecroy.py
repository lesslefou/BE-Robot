...
class lecroy():
    ...
    def channel_setup(self, numberOfAnalogChannels, numberOfMeasurementChannels, analog_ch_dict={}, math_ch_dict={}):
        """
        ...
        for sweep_channels in range(1, (numberOfAnalogChannels+1)):
        ...
        for sweep_channels in range(0, (numberOfMeasurementChannels+1)):
    ...
    def get_analog_channel_setup(self, numberOfAnalogChannels):
        analog_channels = {}
        for sweep_channels in range(1, (numberOfAnalogChannels+1)):
    ...
    def measurement_setup(self, meas_dict,numberOfAnalogChannels):
        """
        ...
        for sweep_measurements in range(1, (numberOfAnalogChannels)):
    ...
    def get_measurement_setup(self,numberOfMeasurementChannels):
        measure_channels = {}
        for sweep_channels in range(1, (numberOfMeasurementChannels)):
    ...
    def saveWaveform(self, dirPath = "D:", channel="C1", fileName="test"):
        '''
        ...
        '''
        self.scope.write('VBS app.SaveRecall.Waveform.WaveformDir = "{}"'.format(dirPath))
        self.scope.write('VBS app.SaveRecall.Waveform.WaveFormat = "Excel"')
        self.scope.write('VBS app.SaveRecall.Waveform.SubFormat = "TimeAmplitude"')
        self.scope.write('VBS app.SaveRecall.Waveform.SaveSource = "{}"'.format(channel))
        self.scope.write('VBS app.SaveRecall.Waveform.TraceTitle = "{}"'.format(fileName))
        self.scope.write('VBS app.SaveRecall.Waveform.DoSave')
        
    def setZoom(self,channel="C1", grid="YT1"):
        '''
        Activate zoom on specific channel and grid
        param: channel = C1 à C4 et Z1 à Zn
        param: grid = YT1 à YT8 en fc du gridMode (Single,Dual, Octo...)
        '''
        self.scope.write('VBS app.Zoom.Z1.View = -1')  #-1=display ON  0=display OFF
        self.scope.write('VBS app.Zoom.Z1.Source = "{}"'.format(channel))
        self.scope.write('VBS app.Zoom.Z1.UseGrid = "{}"'.format(grid))
       

    def setChannelOnGridNumber(self,channel="C1", grid="YT1"): 
        '''
        Set specific channel on specific grid
        param: channel = C1 à C4 
        param: grid = YT1 à YT8 en fc du gridMode (Single,Dual, Octal...)
        '''
        self.scope.write('VBS app.Acquisition.{0}.View = -1')
        self.scope.write('VBS app.Acquisition.{0}.UseGrid = "{1}"'.format(channel,grid))
        
        
        
    def setZoomOnGridNumber(self,zoom="Z1", grid="YT1"): 
        '''
        Set specific zoom on specific grid
        param: channel = Z1 à Zn
        param: grid = YT1 à YT8 en fc du gridMode (Single,Dual, Octal...)
        '''
        self.scope.write('VBS app.Zoom.{0}.View = -1')
        self.scope.write('VBS app.Zoom.{0}.UseGrid = "{1}"'.format(zoom,grid))
        
        
    def getSampleRate(self):
        sampleRate = float(self.scope.query('VBS? return = app.Acquisition.Horizontal.SampleRate').strip())
        return sampleRate
    
    def getChannelVerticalScale(self,channel="C1"):
        ver_scale = self.scope.query('VBS? return = app.Acquisition.{}.VerScale'.format(channel))
        return ver_scale
